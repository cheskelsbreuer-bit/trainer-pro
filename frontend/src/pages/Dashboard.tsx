...
export function Dashboard() {
  ...
  const { data: monthRevenue } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return (data ?? []).reduce((sum, p: Pick<Payment, 'amount'>) => sum + Number(p.amount), 0);
    },
  });
            ...
            <ul className="divide-y divide-slate-100">
              {recentClients.map((c) => (
                <li key={c.id} className="py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={`/clients/${c.id}`} className="font-medium text-slate-900 hover:text-blue-600 truncate block">
                      {c.full_name}
                    </Link>
